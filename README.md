...
Local Kubernetes GitOps lab using Kind or Minikube with ArgoCD watching declarative manifests and reconciling application state.

## Career Progression Story

GitOps step: I moved deployment control from manual kubectl commands into Git-based desired state management.
...
- Shows GitOps knowledge without AWS cost.
- Useful for Kubernetes/platform engineering conversations.
- Simple enough to record screenshots for ArgoCD sync, drift, and rollback.

## Tech Stack

`Kubernetes` `ArgoCD` `GitOps` `Kind` `Minikube` `YAML`
...
flowchart LR
  ...
  ArgoCD --> Cluster[Local Kubernetes Cluster]
  Cluster --> Deploy[Deployment]
  Cluster --> Svc[Service]
  Dev[Developer Change] --> Git
```
...
1. Create a local Kind or Minikube cluster.
2. Install ArgoCD into the cluster.
3. Apply the ArgoCD Application manifest.
4. Change the Kubernetes manifest and watch ArgoCD reconcile the desired state.
